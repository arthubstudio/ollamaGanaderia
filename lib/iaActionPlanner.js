...
function extractUserTarget(raw) {
  ...
  return match?.[1] ? titleValue(cleanValue(match[1])) : "";
}
...
function extractAction(raw) {
  ...
  if (hasAny(t, ["manda", "mandar", "envia", "enviar", "transfiere", "transferir"]) &&
      hasAny(t, ["usuario", "usuaria", "correo"]) && hasAny(t, ["bovino", "vaca", "toro"])) {
    return {
  ...
  if (hasAny(t, ["envia", "enviar", "manda", "mandar"]) && hasAny(t, ["solicitud de amistad", "solicitud de contacto"])) {
    return { tool: "enviarSolicitudAmistad", args: { usuario_destino: extractUserTarget(raw) } };
  }
...
function extractArgsForPending(tool, raw) {
  ...
  if (tool === "enviarSolicitudAmistad") return { usuario_destino: extractUserTarget(raw) || cleanValue(raw) };
...
function detectQuery(raw) {
  ...
  if (hasAny(t, ["que razas", "cuales razas", "listar razas", "lista de razas", "muestrame las razas"])) {
