...
<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      ...
      <NuxtLink to="/vacunas/create" class="bg-black text-white px-6 py-4 rounded-2xl font-semibold">
        Nueva vacuna
      </NuxtLink>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="vacuna in filtered" :key="vacuna.id" class="border-b border-gray-100 hover:bg-gray-50">
            <td class="p-5 font-semibold">{{ vacuna.nombre }}</td>
            <td class="p-5">{{ vacuna.descripcion }}</td>
            <td class="p-5">
              <RowActions
                :view-to="`/vacunas/${vacuna.id}`"
                :edit-to="`/vacunas/${vacuna.id}/edit`"
                :delete-action="() => eliminarVacuna(vacuna.id)"
              />
            </td>
          </tr>
        </tbody>
