  ...
  <div>
    ...
    <div class="grid xl:grid-cols-[1fr_360px] gap-8">
          ...
          <tbody>
            <tr v-for="breed in breeds" :key="breed.id" class="border-t">
              ...
              <td class="p-4 text-right whitespace-nowrap" v-if="canEdit(breed)">
                ...
                <button class="px-3 py-2 text-sm rounded-xl" :class="breed.activo ? 'text-red-700 bg-red-50' : 'text-emerald-700 bg-emerald-50'" @click="toggleBreed(breed)">{{ breed.activo ? "Desactivar" : "Activar" }}</button>
              </td>
      ...
      <form class="bg-white rounded-3xl border border-gray-100 p-6 h-fit space-y-4" @submit.prevent="saveBreed">
        ...
        <button class="w-full bg-black text-white p-3 rounded-xl font-semibold">{{ editing ? "Guardar cambios" : "Crear raza" }}</button>
      </form>
    </div>
  </div>
</template>
